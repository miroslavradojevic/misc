 ...
 * version 21-Nov-2006 Philippe Carl. Adds "Noise tolerance" and "Lower threshold" sliders as well as
 *                     MouseWheelListeners to these sliders.
 */

public class MaximumFinder implements ExtendedPlugInFilter, DialogListener, MouseWheelListener {
	//filter params
	...
	private static int dialogOutputType = SEGMENTED;
	/** output type names */
	...
	private static boolean useMinThreshold = true;
	...
	private Vector    scrollbars;                   // a reference to the Scrollbars of the dialog
	private Vector    numericfields;                // a reference to the NumericFields of the dialog
...
//      gd.addNumericField("Noise tolerance"+unit,tolerance, digits);
		gd.addSlider("Noise tolerance"+unit, 0, ip.getMax(), 10);
		...
		if (thresholded)
		{
			...
			gd.addSlider("Lower threshold", ip.getMin(), ip.getMax(), ip.getMinThreshold());
		}
	...
	public boolean dialogItemChanged(GenericDialog gd, AWTEvent e) {
		...
		if (thresholded)
		{
			...
			imp.getProcessor().setThreshold(gd.getNextNumber(), imp.getProcessor().getMax(), ImageProcessor.RED_LUT);
		}
		else
	...
	{
		e.consume();
		for (int i = 0; i < scrollbars.size(); i++)				// mouseWheelEvents for scrollbar input fields
		{
			if(((Scrollbar)(scrollbars.elementAt(i))).isFocusOwner())
			{
				double value = ((Scrollbar)(scrollbars.elementAt(i))).getValue() + e.getWheelRotation();
				if(value < 0)                           value = 0;
				if(value > imp.getProcessor().getMax()) value = imp.getProcessor().getMax();
				((Scrollbar)(scrollbars   .elementAt(i))).setValue(       (int) value);
				((TextField)(numericfields.elementAt(i))).setText (IJ.d2s(value, 0));
				return;						// The MouseWheelEvent was used, no more action
			}
		}
		for (int i = 0; i < numericfields.size(); i++)			// mouseWheelEvents for numericfields input fields
		{
			if(((TextField)(numericfields.elementAt(i))).isFocusOwner())
			{
				double value = Double.parseDouble(((TextField)(numericfields.elementAt(i))).getText()) + e.getWheelRotation();
				if(value < 0)                           value = 0;
				if(value > imp.getProcessor().getMax()) value = imp.getProcessor().getMax();
				((Scrollbar)(scrollbars   .elementAt(i))).setValue(       (int) value);
				((TextField)(numericfields.elementAt(i))).setText (IJ.d2s(value, 0));
